...
nhanh_144 AS (
  SELECT order_id, created_by_name, sale_name, sale_channel, customer_mobile,
         created_at, order_status, product_price, product_qty, product_discount
...
nhanh_orders_144 AS (
  SELECT
    ...
    -- Doanh thu TẠM TÍNH: doanh thu sản phẩm trước thuế (price*qty - chiết khấu)
    -- cho mọi đơn TRỪ đơn hủy/hoàn. order_status TRỘN mã số (cũ) + chữ TViệt (mới).
    CASE WHEN ANY_VALUE(order_status) NOT IN (
              '58','63','64','71','72','74',
              'Hủy đơn','Hệ thống hủy','HVC hủy','Đã hoàn hàng','Đang chuyển hoàn'
         )
         THEN SUM(SAFE_CAST(product_price AS FLOAT64) * SAFE_CAST(product_qty AS FLOAT64)
                  - IFNULL(SAFE_CAST(product_discount AS FLOAT64), 0))
         ELSE 0
    END AS revenue
  FROM nhanh_144
...
orders_facts AS (
  SELECT
    ...
    order_status,
    CASE
      WHEN order_status IN ('60','Thành công')                         THEN 'thanh_cong'
      WHEN order_status IN ('58','63','64','71','72','74',
                            'Hủy đơn','Hệ thống hủy','HVC hủy',
                            'Đã hoàn hàng','Đang chuyển hoàn')          THEN 'that_bai'
      ELSE 'dang_xu_ly'
    END AS status_group,
    revenue,
